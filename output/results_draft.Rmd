...
Finally, we compared pre- and post-COVID networks. First, to assess their similarity, we calculated a correlation coefficient for the edge-weigths in the two networks (Borsboom et al., 2017; Rhemtulla et al., 2016). Then, we conducted invariance analysis of network structures via *NetworkComparisonTest* R package (van Borkulo et al., 2017). The analysis included an omnibus test that showed whether all edges were exactly the same, followed by post-hoc tests to find specific edges that differed, with the Holm-Bonferroni correction for multiple testing. We also tested whether global strength (the sum of absolute edge-weights in each network), as well as strenght and expected influence of individual nodes differed between the two networks. As a final step, we visualized the cross-sample network with edge-weights averaged between pre- and post-COVID samples.
